...
export async function generateContent({ prompt, documentContext, model = "gpt-4o" }: GenerateContentOptions): Promise<string> {
	...
	const completion = await openai.chat.completions.create({
		...
		messages: [
			{
				role: "system",
				content: "You are a helpful AI assistant that helps users with document editing and writing tasks. Provide clear, concise, and helpful responses."
			},
			{
				role: "user",
