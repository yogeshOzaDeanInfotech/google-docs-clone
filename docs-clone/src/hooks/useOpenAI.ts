...
function parseAIResponseToSlate(content: string): any[] {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (line.includes("|") || (line.includes("Book") && line.includes("Author") && line.includes("Price"))) {
			if (!inTable) {
				inTable = true;
				tableRows = [];
			}
...
export function useOpenAI() {
	...
	const autoEditDocument = async (prompt: string) => {
		...
		try {
			...
			console.log("Current document value:", currentValue);
			console.log("Document store state:", doc);
			
			// Test: Try to manually update the document first
			const testValue = [
				{
					type: "paragraph",
					children: [{ text: "Test: This is a manual update to verify document store is working." }],
				},
				{
					type: "paragraph", 
					children: [{ text: "If you see this, the document store is working correctly." }],
				}
			];
			console.log("Testing manual update with:", testValue);
			doc.setValue(testValue);
			console.log("After manual update:", doc.value);
			
			// Determine the type of edit based on the prompt
			const isAddition = /add|insert|append|include|create|write/i.test(prompt);
			const isModification = /edit|modify|change|update|improve|fix|correct|rewrite|rephrase/i.test(prompt);
			const isReplacement = /replace|substitute|swap/i.test(prompt);
			const isFormatting = /format|style|structure|organize|arrange/i.test(prompt);
...
For a book shop letter with book data table, create a professional letter and include a table with columns like: Book Title, Author, ISBN, Price, Quantity.

Return the content in a clean, well-formatted way.`;
...
Please provide the COMPLETE reformatted version of the document. Apply the requested formatting while keeping all the original content.`;
				...
				// Default case - treat as modification
...
Please provide the edited version of the document. If tables are requested, format them with | separators. Return ONLY the edited content, maintaining the same structure and formatting.`;
			...
			// Parse the AI response and update the document
			const editedContent = res.text.trim();
			console.log("AI Response:", editedContent);
			
			let newValue;
			
			if (isAddition) {
				// For additions, append to existing content
				const parsedContent = parseAIResponseToSlate(editedContent);
				newValue = [...currentValue, ...parsedContent];
			} else {
				// For modifications/replacements, replace the entire content
				newValue = parseAIResponseToSlate(editedContent);
			}

			console.log("New Document Value:", newValue);

			// Update the document with proper error handling
			try {
				...
				const finalValue = doc.value as any[];
				console.log("Final document value after update:", finalValue);
				console.log("Final document value length:", finalValue.length);
				
				// If the update didn't work, try again with a different approach
				if (finalValue.length !== updatedValue.length) {
					console.log("Update failed, trying alternative approach...");
					
					// Try updating with a more explicit approach
					const alternativeValue = newValue.map(item => ({ ...item }));
					doc.setValue(alternativeValue);
					
					// Wait again
					await new Promise(resolve => setTimeout(resolve, 100));
					
					const alternativeFinal = doc.value as any[];
					console.log("Alternative final value:", alternativeFinal);
					
					if (alternativeFinal.length !== newValue.length) {
						// Last resort: try to force update by calling setValue multiple times
						console.log("Trying force update approach...");
						
						// Force multiple updates to trigger re-render
						for (let i = 0; i < 3; i++) {
							doc.setValue([...newValue]);
							await new Promise(resolve => setTimeout(resolve, 50));
						}
						
						const forceFinal = doc.value as any[];
						console.log("Force update final value:", forceFinal);
						
						if (forceFinal.length !== newValue.length) {
							throw new Error("Document update failed after multiple attempts");
						}
					}
				}
			} catch (updateError) {
			...
			// Add system message to show what was done
			const action = isAddition ? "added to" : isModification ? "modified" : isReplacement ? "replaced" : isFormatting ? "reformatted" : "updated";
			...
			// Try fallback: simple text insertion
			try {
				const context = serializeToPlainText(doc.value as any);
				const currentValue = doc.value as any[];
				...
				if (res.ok) {
					...
					const fallbackValue = [...currentValue, ...newParagraphs];
					doc.setValue(fallbackValue);
