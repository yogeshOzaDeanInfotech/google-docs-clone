		...
		>
			...
			<div ref={areaRef} className="flex-1 overflow-auto p-3 space-y-3">
				{messages.map(m => (
					<div
						key={m.id}
						className={`rounded-lg p-2 text-sm ${m.role === "user" ? "bg-blue-50 ml-8" : m.role === "assistant" ? "bg-gray-50 mr-8" : "bg-red-50"}`}
					>
				...
				{loading && (
					<div className="text-xs text-gray-500">OpenAI is typing…</div>
				)}
			</div>
			<div className="p-2 border-t flex items-center gap-2">
				...
				<button
					onClick={handleSend}
					className="h-9 px-3 rounded bg-blue-600 text-white flex items-center gap-1"
				>
					<Send size={16} /> Send
				</button>
			</div>
